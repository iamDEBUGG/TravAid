...
export const analyticsRouter = createRouter({
  spendingTrend: publicQuery
    ...
    .query(({ input }) => {
      ...
      for (const e of filtered) {
        const key = input.granularity === "daily" ? e.expenseDate
          : input.granularity === "monthly" ? e.expenseDate.substring(0, 7)
      ...
      return Array.from(map.entries())
        .map(([period, data]) => ({ period, totalAmount: Math.round(data.total * 100) / 100, transactionCount: data.count }))
  ...
  categoryBreakdown: publicQuery
    ...
    .query(({ input }) => {
      ...
      if (input.startDate) filtered = filtered.filter(e => e.expenseDate >= input.startDate!);
      if (input.endDate) filtered = filtered.filter(e => e.expenseDate <= input.endDate!);
      ...
      return Array.from(map.entries()).map(([category, data]) => ({
        category, totalAmount: Math.round(data.total * 100) / 100,
  ...
  monthlySummary: publicQuery
    ...
    .query(({ input }) => {
      ...
      const filtered = store.expenses.filter(e => e.expenseDate.startsWith(monthStr));
      const totalExpenses = filtered.reduce((s, e) => s + Number(e.amount), 0);
      const totalBudget = store.budgets.reduce((s, b) => s + Number(b.amount), 0);
      const catMap = new Map<string, number>();
      for (const e of filtered) catMap.set(e.category, (catMap.get(e.category) || 0) + Number(e.amount));
      let topCategory = "none";
      let topAmount = 0;
      for (const [cat, amt] of catMap) { if (amt > topAmount) { topCategory = cat; topAmount = amt; } }
      return {
        totalExpenses: Math.round(totalExpenses * 100) / 100,
        totalBudget, budgetUsed: totalBudget > 0 ? Math.round((totalExpenses / totalBudget) * 100) : 0,
        topCategory, transactionCount: filtered.length,
      };
    }),
  ...
  budgetVsActual: publicQuery
    ...
    .query(({ input }) => {
      ...
      return store.budgets.map(b => ({
        category: b.category, budget: Number(b.amount),
  ...
  merchantSummary: publicQuery
    ...
    .query(({ input }) => {
      ...
      return Array.from(map.entries())
        .map(([merchant, data]) => ({ merchant, totalAmount: data.total, transactionCount: data.count }))
  ...
  regionalSummary: publicQuery.query(() => {
    const map = new Map<string, { scores: number[]; crimes: number[]; health: number[] }>();
    for (const c of store.countries) {
    ...
    return Array.from(map.entries())
      .map(([region, data]) => ({
        region, avgScore: avg(data.scores), countryCount: data.scores.length,
        avgCrime: avg(data.crimes), avgHealthcare: avg(data.health),
      }))
