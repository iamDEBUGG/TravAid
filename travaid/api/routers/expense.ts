...
const createExpenseInput = z.object({
  ...
  category: z.enum(["food", "travel", "bills", "entertainment", "shopping", "other"]),
...
const updateExpenseInput = z.object({
  ...
  category: z.enum(["food", "travel", "bills", "entertainment", "shopping", "other"]).optional(),
  description: z.string().max(255).optional(),
  paymentMethod: z.string().max(50).optional(),
  merchant: z.string().max(100).optional(),
...
export const expenseRouter = createRouter({
  create: publicQuery
    ...
    .mutation(({ input }) => {
      const e = store.addExpense({
        userId: 1, amount: input.amount, currency: input.currency,
        category: input.category, description: input.description || null,
        paymentMethod: input.paymentMethod || null, merchant: input.merchant || null,
        status: input.status, expenseDate: input.expenseDate.split("T")[0],
      });
      return { id: e.id };
    }),
  ...
  list: publicQuery
    ...
    .query(({ input }) => {
      ...
      if (input.startDate) items = items.filter(e => e.expenseDate >= input.startDate!);
      if (input.endDate) items = items.filter(e => e.expenseDate <= input.endDate!);
      if (input.search) {
      ...
      items.sort((a, b) => {
        if (input.sortBy === "amount") {
          return input.sortOrder === "asc" ? Number(a.amount) - Number(b.amount) : Number(b.amount) - Number(a.amount);
        }
        if (input.sortBy === "category") {
          return input.sortOrder === "asc" ? a.category.localeCompare(b.category) : b.category.localeCompare(a.category);
        }
        return input.sortOrder === "asc" ? a.expenseDate.localeCompare(b.expenseDate) : b.expenseDate.localeCompare(a.expenseDate);
      });
      const offset = (input.page - 1) * input.limit;
      const paged = items.slice(offset, offset + input.limit);
      return { items: paged, total: items.length, page: input.page, totalPages: Math.ceil(items.length / input.limit) };
    }),
  ...
  getCategories: publicQuery.query(() => {
    ...
    return Array.from(map.entries()).map(([category, data]) => ({
      category, count: data.count, total: Math.round(data.total * 100) / 100,
    }));
  }),
